...
export default function Guidance() {
  ...
  if (insightLoading) {
    return <ScreenSkeleton title={messages.guidance.title} subtitle={messages.guidance.subtitle} />;
  }

  if (!todayInsight) {
    return (
      <ScreenEmptyState
        title={messages.guidance.title}
        subtitle={messages.guidance.subtitle}
        error={insightError}
  ...
  return (
    <PageWrap>
      <PageHero
        eyebrow={messages.guidance.title}
        title={todayInsight.headline}
        subtitle={model.decisionCompass?.lean_in ?? messages.guidance.subtitle}
        accent="mint"
      >
        <div className="grid gap-3 md:grid-cols-2">
          <SceneChip label="Giữ vững" value={model.decisionCompass?.hold_steady ?? 'Giữ phần cam kết quan trọng nhất của hôm nay.'} />
          <SceneChip label="Đừng ép" value={model.decisionCompass?.avoid_force ?? 'Không cần cố giải quyết mọi thứ cùng lúc.'} />
        </div>
      </PageHero>

      <div className="space-y-8">
        <PracticalFocusPanel items={model.practicalGuidance} />
      </div>
    </PageWrap>
  );
}
...
function ScreenSkeleton({
  ...
  return (
    <div className="mx-auto max-w-6xl animate-pulse space-y-8">
      ...
      <div className="h-[420px] rounded-[36px] bg-white/6" />
